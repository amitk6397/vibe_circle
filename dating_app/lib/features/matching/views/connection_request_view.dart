  ...
  void _load() async {
    ...
    try {
      ...
      final pending = list.where(
        (item) => item['status'] == 'pending' && item['receiver_id'].toString() == myId
      ).toList();
  ...
  void _respond(dynamic request, String action) async {
    ...
    try {
      ...
      Get.snackbar(
        ...
        action == 'accept' ? 'You accepted their follow request.' : 'Request removed.',
              ...
              : Column(
                  ...
                  children: [
                    ..._requests.map((item) {
                      ...
                      final localPerson = _discoveryController.people.firstWhereOrNull((p) => p.id == reqId);
                      final String name = localPerson?.name ?? _requesters[reqId]?.name ?? 'User';
                      final String age = localPerson?.age?.toString() ?? _requesters[reqId]?.age?.toString() ?? '18';
                      final String location = localPerson?.city ?? _requesters[reqId]?.city ?? 'Local';
