...
import '../../wallet/models/coin_package_model.dart';

class SubscriptionPlansView extends GetView<WalletController> {
  const SubscriptionPlansView({super.key});

  void _buyCoins(CoinPackage item) async {
    try {
      await controller.buyPackage(item.id);

      final int totalCoins = item.totalCoins;
      Get.defaultDialog(
                  ...
                  : list.isNotEmpty
                      ? GridView.builder(
                          ...
                          itemBuilder: (context, index) {
                            final CoinPackage item = list[index];
                            final bool isPopular = item.isPopular || index == 1;
                            final int purchased = item.purchasedCoins;
                            final int bonus = item.bonusCoins;
                            final int discount = bonus > 0 && purchased > 0 ? ((bonus / purchased) * 100).round() : 0;
