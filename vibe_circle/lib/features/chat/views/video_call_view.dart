...
import 'package:vibe_circle/core/services/agora_call_service.dart';
import 'package:vibe_circle/core/widgets/app_avatar.dart';
import 'package:vibe_circle/features/chat/controllers/chat_controller.dart';
import 'package:vibe_circle/features/profile/controllers/profile_controller.dart';
import 'package:vibe_circle/routes/app_routes.dart';
        ...
        actions: durationOptions.map((item) {
          ...
          return TextButton(
            onPressed: () async {
              Get.back();
              try {
                final session = await _chatController.extendCall(
                  _callId,
                  minutes,
                );
                setState(() {
                  _reservedMinutes =
                      session['reservedMinutes'] ??
                      (_reservedMinutes + minutes);
                });
  ...
  void _reportCall() {
    Get.defaultDialog(
      ...
      onConfirm: () async {
        Get.back();
        try {
          await _profileController.reportUser(
            'call',
            _callId,
          ...
          Get.snackbar(
            'Report submitted',
            'Our safety moderators will review it.',
          );
        } catch (e) {
  ...
  Widget _buildControl(IconData icon, bool active, VoidCallback onTap) {
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Icon(
          icon,
          color: active ? const Color(0xFF101326) : Colors.white,
          size: 20.0,
        ),
      ),
    );
