...
export function CreateCommunityScreen({ navigation }: any) {
  const [name, setName] = useState('');
  const [description, setDescription] = useState('');
  const [category, setCategory] = useState('General');
  const [privacy, setPrivacy] = useState('Public');
  const [privateCommunityCoins, setPrivateCommunityCoins] = useState(0);
  const [rules, setRules] = useState('Be kind. Stay on topic. No spam.');
  const [tags, setTags] = useState('');
  const [location, setLocation] = useState('');
  const [language, setLanguage] = useState('English');
  const [color, setColor] = useState('#5B5CE2');
  const [logo, setLogo] = useState<LocalAttachment | null>(null);
  const [cover, setCover] = useState<LocalAttachment | null>(null);
  const [creating, setCreating] = useState(false);
  const createCommunity = useAppStore((state) => state.createCommunity);
  useEffect(() => {
    void walletApi
      .pricing()
      .then(({ data }) => setPrivateCommunityCoins(data.privateCommunityCoins));
  }, []);
  return (
    <Screen>
      <Header
      ...
      />
      <Text style={ui.h2}>Privacy</Text>
      <View style={ui.wrap}>
        {['Public', 'Private'].map((x) => (
          <Pill key={x} label={x} selected={privacy === x} onPress={() => setPrivacy(x)} />
        ))}
      </View>
      {privacy === 'Private' && (
        <Card>
          <Text style={ui.body}>Admin price: {privateCommunityCoins} coins.</Text>
          <Text style={ui.muted}>
            The community remains discoverable. Users pay once to join and view its content.
          </Text>
        </Card>
      )}
      <Field label="Community rules" value={rules} onChangeText={setRules} multiline />
      <Text style={ui.h2}>Community identity</Text>
      <View style={ui.wrap}>
        {['#5B5CE2', '#2FA89A', '#F06A6A', '#E79B32', '#2878D4'].map((value) => (
          <Pressable
            key={value}
            onPress={() => setColor(value)}
            style={[
              styles.communityColor,
              { backgroundColor: value },
              color === value && styles.communityColorSelected,
            ]}
          />
        ))}
      </View>
      <View style={styles.actionRow}>
        <Button
          title={logo ? 'Logo selected' : 'Choose logo'}
          tone="secondary"
          onPress={() => void pickImage().then(setLogo)}
        />
        <Button
          title={cover ? 'Cover selected' : 'Choose cover'}
          tone="secondary"
          onPress={() => void pickImage().then(setCover)}
        />
      </View>
      {cover && <Image source={{ uri: cover.uri }} style={styles.communityCoverPreview} />}
      <Field
        label="Tags"
        value={tags}
      ...
      <Button
        title="Create community"
        loading={creating}
        disabled={!name.trim() || !description.trim() || !category.trim() || !rules.trim()}
        onPress={async () => {
          setCreating(true);
          try {
            const [uploadedLogo, uploadedCover] = await Promise.all([
              logo ? uploadAttachment(logo) : null,
              cover ? uploadAttachment(cover) : null,
            ]);
            const communityId = await createCommunity({
              ...
              privacy:
                privacy === 'Public' ? 'public' : privacy === 'Private' ? 'private' : 'private',
              premiumPrice: 0,
              rules: rules
                .split(/[.\n]/)
                .map((item) => item.trim())
            ...
            Alert.alert('Community created', `${name.trim()} is ready for its first members.`);
