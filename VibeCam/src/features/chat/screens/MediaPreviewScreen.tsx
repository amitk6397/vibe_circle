...
export function MediaPreviewScreen({ navigation, route }: any) {
  const [attachment, setAttachment] = useState<LocalAttachment | null>(
    route.params?.attachment || null,
  );
  return (
    <Screen>
      <Header
      ...
      {attachment ? (
        <Card style={styles.mediaPreviewCard}>
          {attachment.kind === 'image' ? (
            <Image source={{ uri: attachment.uri }} style={styles.mediaPreviewImage} />
          ) : (
        ...
        <View style={styles.media}>
          ...
          <Text style={ui.h2}>Choose media to preview</Text>
      ...
      <Button
        title={attachment ? 'Choose another image' : 'Choose an image'}
        icon="images"
        onPress={async () => setAttachment(await pickImage())}
      />
      <Button
        title="Choose a file"
        tone="secondary"
        icon="document-attach"
        onPress={async () => setAttachment(await pickDocument())}
      />
      {attachment && (
        <Button title="Close preview" tone="ghost" onPress={() => navigation.goBack()} />
      )}
    </Screen>
  );
}
