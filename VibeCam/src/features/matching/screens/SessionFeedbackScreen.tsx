...
export function SessionFeedbackScreen({ navigation, route }: any) {
  ...
  return (
    <Screen>
      <Header
        ...
        subtitle="Feedback is private and improves suggestions."
      ...
      <View style={styles.stars}>
        {[1, 2, 3, 4, 5].map((x) => (
          <Pressable key={x} onPress={() => setRating(x)}>
            <Ionicons
              name={x <= rating ? 'star' : 'star-outline'}
              size={36}
      ...
      <Text style={ui.h2}>What went well?</Text>
      <View style={ui.wrap}>
        {['Helpful', 'Respectful', 'Easy to talk to', 'Good listener', 'Shared interests'].map(
          (x) => (
            <Pill
              key={x}
              label={x}
              selected={tags.includes(x)}
              onPress={() => setTags(tags.includes(x) ? tags.filter((y) => y !== x) : [...tags, x])}
            />
          ),
        )}
      </View>
        ...
        onPress={async () => {
          ...
          try {
            if (route.params?.matchId)
              await matchingApi.feedback(route.params.matchId, rating, tags);
            Alert.alert('Thank you', 'Your private feedback was saved.');
      ...
      <Button
        ...
        tone="danger"
        onPress={() =>
          Alert.alert('Safety actions', 'Choose what you need.', [
            { text: 'Cancel', style: 'cancel' },
            {
              text: 'Block user',
              style: 'destructive',
              onPress: () => {
                if (route.params?.personId) useAppStore.getState().blockUser(route.params.personId);
              },
            },
            {
              text: 'Report session',
              ...
              onPress: () => {
                if (route.params?.personId)
                  void safetyApi
                    .report('user', route.params.personId, 'Unsafe Connect session')
