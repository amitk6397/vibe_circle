  ...
  return (
    <Screen>
      ...
      <View style={styles.profile}>
        <Avatar name={profile.name} uri={profile.avatarUri} size={96} />
        <Text style={ui.title}>{profile.name}</Text>
        <Text style={ui.muted}>
          @{profile.username} · {profile.city}
        </Text>
      </View>
      <Card>
        <Text style={ui.body}>{profile.bio}</Text>
      </Card>
      <View style={styles.stats}>
        <Stat value={posts.length} label="Posts" />
        <Stat value={followerCount} label="Followers" />
        <Stat value={joinedCommunities.length} label="Communities" />
      </View>
      <Text style={ui.h2}>Badges</Text>
      <View style={ui.wrap}>
        <Pill label="Helpful human" selected />
        <Pill label="Good listener" />
        <Pill label="Early member" />
      </View>
    </Screen>
  );
}
