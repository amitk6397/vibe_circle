...
export function AccountManagementScreen({ navigation }: any) {
  const profile = useAppStore((s) => s.profile);
  return (
    <Screen>
      <Header
        title="Account management"
        subtitle={profile.email || 'Private account email'}
      ...
      <Menu
        ...
        onPress={() =>
          void accountApi
            .logoutAll()
            .then(() => Alert.alert('Sessions revoked', 'All sessions have been signed out.'))
            .catch((error) => Alert.alert('Could not revoke sessions', error.message))
        }
      />
      <Button
        title="Log out"
            ...
            [
              { text: 'Cancel', style: 'cancel' },
              {
                ...
                onPress: () =>
                  void accountApi
                    .delete()
                    .then(() => tokenStorage.clear())
                    .then(() => {
                      useAppStore.setState({ authenticated: false });
                      navigation.reset({ index: 0, routes: [{ name: 'Login' }] });
                    })
                    .catch((error) => Alert.alert('Could not delete account', error.message)),
              },
            ],
          )
        }
