...
export function ConnectionsScreen({ navigation }: any) {
  ...
  useEffect(() => {
    usersApi
      .connections()
      .then(({ data }) => setConnections(data.filter((item) => item.status === 'accepted')))
      .catch((reason) => setError(reason.message || 'Could not load connections.'))
  ...
  const connectedPeople = connections
    ...
    .map((item) =>
      people.find(
        (person) =>
          person.id ===
          (item.requester_id === currentUserId ? item.receiver_id : item.requester_id),
      ),
    )
    .filter(Boolean);
  return (
    <Screen>
      <Header
        title={tab}
        subtitle={tab === 'Followers' ? 'People who follow you' : 'People you follow'}
      ...
      <View style={ui.wrap}>
        ...
        />
        <Pill label="Requests" onPress={() => navigation.navigate('ConnectionRequest')} />
      </View>
      {loading && <ChatSkeleton />}
      {!!error && (
        <EmptyState icon="cloud-offline-outline" title="Could not load connections" text={error} />
      )}
      {!loading && !error && !connectedPeople.length && (
        <EmptyState
          icon="people-outline"
      ...
      {connectedPeople.map((x: any) => (
        <PersonCard
          key={x.id}
          person={x}
          onPress={() => navigation.navigate('PublicProfile', { personId: x.id })}
        />
      ))}
    </Screen>
  );
